...
void loadVertexCoords(struct tiling* pG){
  ...
  sprintf(coordFile, "saved_tilings/%s_coordinates.txt", pG->type);
...
void loadEdges(struct tiling* pG){
  ...
  sprintf(edgeFile, "saved_tilings/%s_edges.txt", pG->type);
...
void loadFaces(struct tiling* pG){
  ...
  sprintf(faceFile, "saved_tilings/%s_faces.txt", pG->type);
...
void loadOpenEdges(struct tiling* pG){
  ...
  sprintf(doEdgeFile, "saved_tilings/%s_open_edges.txt", pG->type);
...
void ListTilings(char* dirName){
  ...
  if(dir != NULL){
    // Print the list of all face files in the directory
    char* tileName;
    int ci = 0;
    while((ent = readdir(dir)) != NULL){
      if(strlen(ent->d_name) > 10){
	tileName = ((ent->d_name) + (strlen(ent->d_name) - 10));
	if(strncmp(tileName, "_faces.txt", 10) == 0){
