...
//add a face given as an ordered list of vertices
...
/*
//=======================================
//		Add a vertex to a tiling
//=======================================

//add the vertex of coordinate (x, y) to the tiling G
void AddVertex(struct tiling *G, int x, int y)
{
    int i;
    
    //check if the vertex is new
    int isNew = 1;
	for (i=0; i<(*G).v; i++)
        if ( ((fabs(x - (*G).Coord[i][0]) < epsilon) && (fabs(x - (*G).Coord[i][1]) < epsilon))||((fabs(x - (*G).Coord[i][1]) < epsilon) && (fabs(x - (*G).Coord[i][0]) < epsilon)) )
            isNew = 0;

    //add vertex if it is new
    if (isNew)
    {
        (*G).v++;
        float **nextCoord = malloc(((*G).v)*sizeof(float *));
        for (i=0; i<(*G).v; i++)
            nextCoord[i] = malloc(2*sizeof(float));
        
        for (i=0; i<(*G).v-1; i++)
        {
            nextCoord[i][0] = (*G).Coord[i][0];        
            nextCoord[i][1] = (*G).Coord[i][1];
        }
        nextCoord[(*G).v][0] = x;
        nextCoord[(*G).v][1] = y;
...
//add a list of vertices of coordinate (x=vertexList[i][0], y=vertexList[i][1]) to the tiling G
void AddVertexList(struct tiling *G, int **vertexList, int listSize)
{
    int i;
    int k, x, y;
    int *isNew = malloc(listSize*sizeof(int));
    ...
    //count new vertices
    int nbOfNewVertices = 0;
    for (k=0; k<listSize; k++)
        if (isNew[k])
            nbOfNewVertices++;
    
    //add new vertices coordinates
    if (nbOfNewVertices > 0)
    {
        ...
        for (i=0; i<(*G).v; i++)
            nextCoord[i] = malloc(2*sizeof(float));
        for (i=0; i<(*G).v-nbOfNewVertices; i++)
        {
            nextCoord[i][0] = (*G).Coord[i][0];        
            nextCoord[i][1] = (*G).Coord[i][1];
        }
        ...
        //add new vertices coordinates
        k = 0;
        for (i=(*G).v-nbOfNewVertices; i<(*G).v; i++)
        {
            //find next new vertex
            while (!isNew[k])
                k++;
                
            //add new vertex k
            {
                nextCoord[i][0] = vertexList[k][0];
                nextCoord[i][1] = vertexList[k][1];
            }
        }
