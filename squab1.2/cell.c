...
//==================================
//	TRANSLATION OF A CELL VERSION 2
//==================================


//compute the coordinate of the center of the face f of the tiling G
float *FaceCenter(struct tiling G, int f)
{
    int j;
    int ej, ux, uy, vx, vy;
    float *center = malloc(2*sizeof(float));
    ...
    {
        ej = G.F[f][j];
        ux = G.Coord[G.E[ej][0]][0]/(2*G.F[f][0]);
        vx = G.Coord[G.E[ej][1]][0]/(2*G.F[f][0]);
        uy = G.Coord[G.E[ej][0]][1]/(2*G.F[f][0]);
        vy = G.Coord[G.E[ej][1]][1]/(2*G.F[f][0]);
        center[0] += ux + vx;
        center[1] += uy + vy;
    }
...
//compute the coordinate of the barycenter of a list of vertices of the tiling G
//vertexList[0] is the size of the list
float *Barycenter(struct tiling G, int *vertexList)
{
    int j;
    float *center = malloc(2*sizeof(float));
    center[0] = 0;
    center[1] = 0;
    for (j=1; j<=vertexList[0]; j++)
    {
        center[0] += G.Coord[j][0]/(vertexList[0]);
        center[1] += G.Coord[j][1]/(vertexList[0]);
    }
...
// return he index of the vertex of coord (x, y)
// return -1 if the vertex does not exist
int FindVertexIndex(struct tiling G, float x, float y)
{
    int i;
    for (i=0; i<G.v; i++)
        if ( ((fabs(G.Coord[i][0] - x) < epsilon) && (fabs(G.Coord[i][1] - y) < epsilon))
           || ((fabs(G.Coord[i][1] - x) < epsilon) && (fabs(G.Coord[i][0] - y) < epsilon)) )
            return i;
    return -1;
}
...
//return a tiling obtained by translating a generating cell 
//n-1 time in the direction t
//we obtain n copies of the original cell
struct tiling PeriodicTiling(struct tiling C, float *t, int n)
{
    int i, j, k;
    ...
    //add the vertices of the n translated cells
    //new faces
    int **vertexList;
    for (i=0; i<n; i++)
    {
        vertexList = malloc(C.v*sizeof(int *));
        for (j=0; j<C.f; j++)
            vertexList[j] = malloc(2*sizeof(int));
        for (j=0; j<C.f; j++)
        {
            vertexList[j][0] = C.Coord[j][0] + i*t[0];
            vertexList[j][1] = C.Coord[j][1] + i*t[1];
        }
    }
    AddVertexList(&G, vertexList, C.v);
            
    //add the faces of the n translated cells
    //check first that the face is new
    int isNew;
    int *faceVertexList;
    int v, translatedVertex;
    for (i=0; i<n; i++)
    {
        for (j=0; j<C.f; j++)
        {
            //start with the list of vertices of the face of C and translate each vertex
            faceVertexList = FaceToVertexList(G, G.F[j]);
            for (k=1; k<=C.F[j][0]; k++)
            {
                v = faceVertexList[k];
                translatedVertex = FindVertexIndex(G, C.Coord[v][0] + i*t[0], C.Coord[v][1] + i*t[1]);
                faceVertexList[k] = translatedVertex;
            }
            //test if the face is new
            isNew = 1;
            k = 0;
            while ( (isNew) && (k<G.f) )
                if (fabs(FaceCenter(G, k) - Barycenter(G, faceVertexList)) < epsilon)
                    isNew = 0;
            if (isNew)
                AddFace(&G, &faceVertexList[1], faceVertexList[0]);
        }
    }
